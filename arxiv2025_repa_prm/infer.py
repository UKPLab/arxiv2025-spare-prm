...
def create_messages_batch(
    ...
    for i, ex in enumerate(batch):
        ...
        msgs = create_prompt_messages(
            ...
            system_content=instruction if instruction else ex.get(instruct_field),
...
def main(exp, run=None):
    ...
    msg_ex_args = exp["messages"]["creation_args"]
    ex_to_content = partial(example_to_content, **msg_ex_args)
    ...
    for name, dataset in tqdm(datasets.items()):
        ...
        if (filter_kwargs := exp["data"].get("filter")) and not filter_kwargs.get("model_output"):
            dataset[infer_split] = dataset[infer_split].filter(
                partial(field_filter, **filter_kwargs)
            )
        ...
        msgs_batch_creator = partial(
            ...
            instruction=exp["messages"].get("instruction"),
            instruct_field=exp["messages"]["instruct_field"],
            exemplar_sampler=exemplar_sampler,
            system_as_separate=exp["messages"].get("system_as_separate", True),
            system_user_sep=exp["messages"].get("system_user_sep", "\n\n"),
        )
        ...
        for i, batch_start_idx in enumerate(tqdm(range(0, len(dataset), batch_size))):
            ...
            out = [
                dict(
                    ...
                    context_id=ex.get(exp["messages"]["context_id"]),
                    question_id=ex.get(exp["messages"].get("question_id")),
                    targets=ex[msg_ex_args["response_field"]],
                    target_aliases=ex.get(msg_ex_args.get("response_aliases"), []),
                    target_choices=ex.get(msg_ex_args.get("response_choices"), []),
                    model_output=resp,
                )
