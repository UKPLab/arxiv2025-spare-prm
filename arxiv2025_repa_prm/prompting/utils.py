...
def create_prompt_messages(
    ...
    msgs = []
    if system_content and system_as_separate:
        system_msg = {role_phrase: system_role, content_phrase: system_content}
        msgs.append(system_msg)

    for i, inp in enumerate(contents):
        
        content_entry = inp[user_key]
        if i == 0 and not system_as_separate and system_content:
            content_entry = system_content + system_user_sep + content_entry
        
        user_msg = {role_phrase: user_role, content_phrase: content_entry}
